...
public class BaseJobLogManagerImpl implements JobLogManager {
	...
	private Runnable taskFor(final Job job, final int gravity, final String message) {
		return new Runnable() {
			...
			public void run() {
				...
				// save
				ResourceWriter<JobLog> jobLogWriter = resourceManager.getEntityWriter(job, JobLog.class);
	...
	public JobLog lookup(String contextID, String name, String user, int number) {
		...
		ResourceReader<JobLog> jobLogReader = resourceManager.getEntityReader(jobCaller, JobLog.class);
