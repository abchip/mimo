...
public class HttpEntityProviderImpl extends EntityProviderImpl {
	...
	private boolean isNote(Frame<?> frame) {
		return frame.getSuperNames().contains(BizEntityNote.class.getSimpleName());
	}
	...
	public <E extends EntityNameable> EntityReader<E> doEntityReader(ContextProvider contextProvider, Frame<E> frame, String resource) {
		...
		if (isNote(frame))
			entityReader = (EntityReader<E>) new HttpEntityNoteReaderImpl<BizEntityNote>((EntitySerializer<BizEntityNote>) entitySerializer, resource);
		else
			entityReader = new HttpEntityReaderImpl<E>(entitySerializer, resource);
	...
	public <E extends EntityNameable> EntityWriter<E> doEntityWriter(ContextProvider contextProvider, Frame<E> frame, String resource) {
		...
		EntityWriter<E> entityWriter = null;
		EntitySerializer<E> resourceSerializer = resourceManager.createEntitySerializer(contextProvider, frame, SerializationType.JAVA_SCRIPT_OBJECT_NOTATION);
		if (isNote(frame))
			entityWriter = (EntityWriter<E>) new HttpEntityNoteWriterImpl<BizEntityNote>((EntitySerializer<BizEntityNote>) resourceSerializer, resource);
		else
			entityWriter = (EntityWriter<E>) new HttpEntityWriterImpl<E>(resourceSerializer, resource);
