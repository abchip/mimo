...
public class ExportPartyVcardServlet extends BaseServlet {
	...
	protected void execute(ContextProvider contextProvider, HttpServletRequest request, HttpServletResponse response)
			throws IOException {
		_execute(contextProvider, request, response);
	}

	private <E extends EntityNameable> void _execute(ContextProvider contextProvider, HttpServletRequest request,
		...
		EntityReader<PartyGroup> partyGroupReader = resourceManager.getEntityReader(contextProvider, PartyGroup.class);
...
//			return;
	...
	private void writeLatestPostalAddress(ContextProvider contextProvider, ezvcard.VCard vcard, String partyId) {
		...
		EntityReader<PartyContactMech> partyContactMechReader = resourceManager.getEntityReader(contextProvider, PartyContactMech.class);
		EntityReader<PostalAddress> postalAddressReader = resourceManager.getEntityReader(contextProvider, PostalAddress.class);

		try (EntityIterator<PartyContactMech> partyContactMechList = partyContactMechReader.find(filter, null, 100)) {
			for (PartyContactMech partyContactMech : partyContactMechList) {
				EntityReader<ContactMech> contactMechReader = resourceManager.getEntityReader(contextProvider, ContactMech.class);
	...
	private void writeLatestTelecomNumber(ContextProvider contextProvider, VCard vcard, String partyId) {
		...
		String filter = "partyId = \"" + partyId + "\"  AND thruDate IS NULL";
		EntityReader<PartyContactMech> partyContactMechReader = resourceManager.getEntityReader(contextProvider, PartyContactMech.class);
		EntityReader<TelecomNumber> telecomNumberReader = resourceManager.getEntityReader(contextProvider, TelecomNumber.class);
